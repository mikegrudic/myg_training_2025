...
for i, f in enumerate(sorted(runs)):
    ...
    #    print(distances.max())
    if distances.max() < 4.9e3:  # or distances.max() > 6e3:
        continue
    ...
    dt_program = timestamps[-1].date() - datetime(2025, 1, 1).date()
    # print(dt.days)
    ...
    pace = pace[distances < 5e3]
    heartrates = heartrates[distances < 5e3]
